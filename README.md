...
* **Mirror Matches:** Bringing the exact same `.fleet` file as your opponent is supported. The mod specifically targets a combination of your Player Name and Ship key to ensure your battle damage doesn't apply to the enemy's identical ship. A consequence of this is that you NEED to keep the same ship key for mod to function, either by keeping to the same ship in ship editor, or by enforcing it manually *v